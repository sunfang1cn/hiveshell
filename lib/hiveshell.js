...
adaptor['json'] = function (input, rs, cs) {
  ...
  var output = [];
  for (var i = 1, l = row.length - 1; i < l; i++) {
    var col = row[i].split(cs);
    if (col.length === header.length) {
      output[i - 1] = {};
      for (var j = 0; j < col.length; j++) {
        output[i - 1][header[j]] = col[j];
      }
    }
  }
  return output;
...
module.exports = function (conf) {
  ...
  this.excute = function (sql, args, cb) {
    if (!sql) {
    ...
    summary.end = summary.write = function () {
      ;
    };
    var proc = ps('hive -e \"' + sql + '\"').data(function (err, stdout, stderr) {
      if (err) {
        return cb(new Error("Hiveshell: Hive Excute Error! " + stderr.toString()));
      }
      ...
      return cb(null, adaptor[conf.output](out, conf.rowSplit, conf.colSplit));
    });
    proc.pipe(summary);
  }
  return this;
